...
category <- xpathSApply(webpage, "//div[@class='rubber__fixed']/div/ul/li/a", xmlGetAttr, "href");
category = category[-1]
category

main1 = substr(main, 1, 22)
...
for(i in 1:length(category))
{
  ...
  #{
    ...
    news_page <- xpathSApply(tmp, "//h2[@class='story__title']//a[@href]", xmlGetAttr, "href")
    # news_page
  ...
  for(j in 1:length(ref))
  {
    ...
    news1_text <- xpathSApply(news1_page, "//div[@class='poem-text']//h2", xmlValue)
    
    if(length(news1_text) != 0){
      news1_text <- xpathSApply(news1_page, "//div[@class='poem-text']//p[following-sibling::style]", xmlValue)
    }else
    {
      news1_text <- xpathSApply(news1_page, "//div[@class='poem-text']", xmlValue);
    }
